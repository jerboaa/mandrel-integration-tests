...
public enum WhitelistLogLines {
    ...
    IMAGEIO_BUILDER_IMAGE {
        ...
        public Pattern[] get(boolean inContainer) {
            return new Pattern[] {
                    ...
                    // Ubi
                    Pattern.compile(".*Error: copying system image from manifest list.*"),
            };
        }
    },
    ...
    FOR_SERIALIZATION {
        ...
        public Pattern[] get(boolean inContainer) {
            ...
            if ((UsedVersion.getVersion(inContainer).compareTo(Version.create(23, 1, 11)) >= 0) &&
                (UsedVersion.getVersion(inContainer).compareTo(Version.create(24, 0, 0)) < 0)) {
                // See: https://github.com/Karm/mandrel-integration-tests/issues/400
                // Warning: "customTargetConstructorClass" is deprecated in serialization-config.json...
                // customTargetConstructorClass elements are being generated by the native image agent
                // in serialization-config.json before native-image runs.
                p.add(Pattern.compile(".*Warning: \"customTargetConstructorClass\" is deprecated in serialization-config.json\\. .*"));
            }
            if ((UsedVersion.getVersion(inContainer).compareTo(Version.create(25, 0, 0)) >= 0)) {
                // GraalVM 26 or graal/master that is Labs JDK 25 based adds a warning count at the end of the build output.
                // See https://github.com/oracle/graal/pull/12162
